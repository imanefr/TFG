...
$titulo_dashboard = "Dashboard Plan de Igualdad";
...
if ($_POST && isset($_POST['accion'])) {
    switch ($_POST['accion']) {
        case 'eliminar':
            $id = (int) $_POST['id'];
            $stmt = $conexion->prepare("DELETE FROM plan_igualdad WHERE id = ?");
        ...
        case 'nueva':
            ...
            $stmt = $conexion->prepare("INSERT INTO plan_igualdad (titulo, texto, link, texto_link, imagen, video, pdf, fecha_publicacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        case 'editar':
            ...
            if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($file_extension, $allowed)) {
                    $new_filename = 'plan_igualdad_' . time() . '_' . rand(1000, 9999) . '.' . $file_extension;
                    $upload_path = $upload_dir . $new_filename;
...
if (isset($_GET['editar'])) {
    ...
    $stmt = $conexion->prepare("SELECT * FROM plan_igualdad WHERE id = ?");
...
<head>
    ...
    <title>Gestión Plan de Igualdad - Dashboard Admin</title>
    ...
    <div class="dashboard_erasmus_container">
        ...
        <?php else: ?>
            ...
            <div class="dashboard_erasmus_seccion_form <?php echo $modo_edit ? 'dashboard_erasmus_modo_edit' : ''; ?>">
                <h2>
                    <?php if ($modo_edit): ?>
                        <i class="fas fa-edit"></i> Editar Sección (ID: <?php echo $noticia_edit['id']; ?>)
                    <?php else: ?>
                        ...
                        <?php foreach ($noticias as $noticia): ?>
                            <div class="dashboard_erasmus_noticia_card">
                                ...
                                <div class="dashboard_erasmus_noticia_medios">
                                    <?php if ($noticia['link']): ?>
                                        <a href="noticias_relevantes.php?id=<?php echo $noticia['id']; ?>" class="dashboard_erasmus_noticia_enlace">
